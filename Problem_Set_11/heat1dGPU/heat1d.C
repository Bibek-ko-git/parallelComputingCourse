...
{
    printf("Usage: heat1d [Nx] [tFinal] [debug] [matlabFileName.m]\n"
           "       Nx  = number of grid cells.\n"
           "       tFinal = final time for solution.\n"
           "       debug = 1 to generate debug data or 0 to not. \n"
    ...
    if (argc>=2)  // read any command line arguments
    {
        ...
        {
            tFinal = atof(argv[2]);
            printf("Setting tFinal = %f\n",tFinal);
            if (argc>=4) 
            {
                debug = atoi(argv[3]);
                printf("Setting debug flag to %d\n",debug);
                if (argc>=5)
                {
                    matlabFileName = argv[4];
    ...
    printf("CPU: numSteps=%4d, Nx=%3d, maxErr=%9.2e, cpu=%9.2e(s)\n",numSteps,Nx,maxErr,cpuTimeStep);
    ...
    rpar_h[RPAR_RX] = rx;
    rpar_h[RPAR_KX] = kx;
    rpar_h[RPAR_KXPI] = kxPi;
    rpar_h[RPAR_KAPPOPISQ] = kappaPiSq;
    ...
    Real *u_initial = new Real[nd1];
    for (int i = nd1a; i <= nd1b; i++) {
        u_initial[i - nd1a] = UTRUE(x(i), 0.0);
    }
    
    // Call GPU solver
    Real gpuTime = 0.0;
    int Nt_gpu = 0;
    Real gpuMaxErr = SolveHeat1dGPU(u_initial, numSteps, tFinal, ipar_h, rpar_h, x_p, gpuTime, Nt_gpu, debug);
