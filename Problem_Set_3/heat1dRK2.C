...
int main (int argc, char* argv[])
{
    ...
    { 
        ...
        //-----------------RK2 implementation--------------------
        // Adding temporary array for RK2 intermediate step
        Real *w_k = new Real [nd1];
        #define w(i) w_k[i-nd1a]
        
        // --- update the interior points ----
        // RK2 Step 1: compute for k1 at t+dt/2
        for( int i=n1a; i<=n1b; i++ )
        {
            w(i) = uc(i) + 0.5*( rx*(uc(i+1) - 2.*uc(i) + uc(i-1) ) + dt*FORCE( x(i),t ));
        }
        ...
        for( int side=0; side<=1; side++ )
        {
            ...
            {
                w(i) = UTRUE(x(i),t+dt/2.0);
                w(i-is) = 3.*w(i) - 3.*w(i+is) + w(i+2*is); // extrapolate ghost
            }
            else
            {// Neumann BC
                w(i-is) = w(i+is) - 2.*is*dx*UTRUEX(x(i),t+dt/2.0);
            }
        }

        // RK2 Step 2 : Compute final solution using k1
        for ( int i=n1a; i<=n1b; i++ )
        {
            un(i) = uc(i) + ( rx* ( w(i+1) - 2.*w(i) + w(i-1)) + dt * FORCE( x(i), t + dt/2.0));
        }
        
        // Apply boundary condtion at t + dt
        for(int side = 0; side <= 1; side++)
        {
            ...
            {
                un(i) = UTRUE(x(i), t+dt);
                un(i-is) = 3.*un(i) - 3.*un(i+is) + un(i+2*is);
            }
            else
            {
