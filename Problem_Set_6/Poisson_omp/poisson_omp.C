...
int jacobiIteration( RealArray & u, RealArray & f, PoissonParameters & par )
{
    ...
    // Real res0 = getMaxResidual( ua[current],f,h );
...
int gaussSeidelIteration( RealArray & u, RealArray & f, PoissonParameters & par )
{
    ...
    const int numIterations=n;
    // printf("GS: numIterations=%d, nx=%d, ny=%d, cpu time= %9.2e (s)\n",numIterations,nx,ny,cpu);
    if( maxRes<tol )
    {
...
// ---------------------------------------------
// int redBlackIteration( RealArray & u, RealArray & f, Real xa, Real ya, Real dx[], int maxIterations, Real tol, RealArray & uTrue )
...
int conjugateGradientIteration( RealArray & u, RealArray & f, PoissonParameters & par )
{
    ...
    // rNormSquared = r^T r
    rNormSquared=0.;
    #pragma omp parallel for reduction(+:rNormSquared)
    for( int i2=n2a+1; i2<=n2b-1; i2++ )
    for( int i1=n1a+1; i1<=n1b-1; i1++ )
    {
    ...
    for( n=0; n<maxIterations; n++ )
    {
        ...
        #pragma omp parallel for reduction(+:rNormSquaredNew)
        for( int i2=n2a+1; i2<=n2b-1; i2++ )
            for( int i1=n1a+1; i1<=n1b-1; i1++ )
            {
        ...
        // // rNormSquaredNew = r^T r *MERGE WITH PREVIOUS LOOP ??
        // rNormSquaredNew=0.;
        // for( int i2=n2a+1; i2<=n2b-1; i2++ )
        //   for( int i1=n1a+1; i1<=n1b-1; i1++ )
        //   {
...
int main(int argc, char *argv[])
{
    ...
    string itrMethod = "RB"; // default
    par.itrMethod = &itrMethod;
    ...
    for( int i=1; i<argc; i++ )
    {
        ...
        else if( parseCommand( line, "-itrMethod=",itrMethod)) {}
    }
    ...
    // // f = - Delta (u)
    // #define FORCE(x,y) (kxpkxp +kypkyp)*UTRUE(x,y)
    ...
    int numThreadCounts = 7;

    for (int itr=0; itr<numThreadCounts; itr++)
    {
        int numThreads = threadCounts[itr];
        omp_set_num_threads(numThreads);
        printf("=============================================================\n");
        printf("Number of threads = %d\n",numThreads);
        par.numThreads = numThreads;

        // initial guess of each method 
        u=0.;
        ...
        // ===== Gauss-Seidel =====
        else if ( itrMethod=="GS" )
        {
            gaussSeidelIteration( u, f, par );
        }

        // ===== Red-Black Gauss-Seidel =====
        else if ( itrMethod=="RB" )
        {
            redBlackIteration( u, f, par );
        }

        // ==== Conjugate Gradient =========
        else if ( itrMethod=="CG" )
        {
        ...
        {
            printf("ERROR: Unknown iteration method [%s]\n",itrMethod.c_str());
            printf("Options are: Jac: Jacobian, GS: Gauss Seidel, RB:Red-Black, CG: Conjugate Gradient\n");
            exit(1);
        }
    }
    return 0;
